...
const HTML_PATH = join(__dirname, '58-Transformer原理3D直觉.html')
...
function startServer(port = 8768) {
  const baseDir = __dirname
  const server = createServer((req, res) => {
    const filePath = join(baseDir, decodeURIComponent(req.url === '/' ? '/58-Transformer原理3D直觉.html' : req.url))
...
async function main() {
  ...
  // Scenes that have action animations needing extra warmup frames
  const ACTION_SCENES = new Set([4, 5, 6, 7])
  const EXTRA_WARMUP = 30
  ...
  for (let frame = 0; frame < totalFrames; frame++) {
    ...
    for (const seg of segments) {
      if (frame === seg.frameStart && seg.step >= 0 && seg.step !== currentStep) {
        ...
        // 切换步骤后多渲染几帧让粒子系统启动
        const warmup = ACTION_SCENES.has(seg.step) ? EXTRA_WARMUP : 10
  ...
  const audioPath = join(WORK_DIR, '58-full-narration.mp3')
  const outputPath = join(OUTPUT_DIR, '58-Transformer原理3D直觉.mp4')
