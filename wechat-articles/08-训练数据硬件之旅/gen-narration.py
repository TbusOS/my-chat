"""
生成 60-大模型训练数据硬件之旅 的分段配音，输出 timing.json
"""
import asyncio
import json
import os
...
async def generate_audio(seg):
    """生成单段配音"""
    out_path = os.path.join(WORK, f"{seg['id']}.mp3")
    ...
    return {"id": seg["id"], "step": seg["step"], "duration": duration}
...
async def main():
    ...
    timing = []
    for seg in SEGMENTS:
        info = await generate_audio(seg)
        timing.append(info)
        print(f"  {info['id']:>10s}  step={info['step']:>2d}  {info['duration']:.1f}s")
    ...
    full_path = os.path.join(WORK, "60-full-narration.mp3")
    subprocess.run([
        "ffmpeg", "-y", "-f", "concat", "-safe", "0",
        "-i", list_path, "-c:a", "libmp3lame", "-q:a", "2", full_path
    ], check=True)
